...
| 코드 스니펫                                         | 내용 (설명)                                                                               | 주요 역할          |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------- | ------------------ |
| `const header = document.querySelector(".header");` | `.header` 클래스를 가진 HTML 요소를 변수 `header`에 할당합니다.                           | DOM 요소 선택      |
...
이 기능을 통해 사용자에게 현재 페이지 상단에 위치하고 있는지 (기본 헤더) 또는 콘텐츠 탐색 중인지 (스크롤된 헤더)를 시각적으로 명확하게 알려줄 수 있습니다.
